...
if __name__ == '__main__':
    ...
    for r0 in r0s:
        ...
        f = h5py.File(f'marginal_r0_{r0}.h5', 'w')
        ...
        for i, sigma in enumerate(noise):
            ...
            decSI = torchmfbd.Deconvolution('marginal.yaml')
            ...
            if i == 0:
                ...
                dpars_s0 = f.create_dataset(f'pars_s0', shape=(n_noise, 4), dtype='float32')
                drho = f.create_dataset(f'rho', shape=(n_noise, nx, ny), dtype='float32')
                dloss = f.create_dataset(f'loss', shape=(n_noise,n_iter), dtype='float32')

            dbest_frame[i, ...] = best_frame
            dobj[i, ...] = obj
            dpars_s0[i, ...] = decSI.pars_s0[0].cpu().numpy()
            drho[i, ...] = decSI.rho[0].cpu().numpy()
            dloss[i, ...] = decSI.loss.cpu().numpy()
