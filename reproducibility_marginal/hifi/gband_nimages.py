...
if __name__ == '__main__':
    ...
    to_do = ['marginal']#'marginal']

    s_u = 1000.0

    if 'joint' in to_do:
        for n_frames in [2, 10, 50]:
            ...
            hdul.writeto(f'gband_joint_nimages_{n_frames}_su_{s_u}.fits', overwrite=True)
    ...
    if 'marginal' in to_do:
        for n_frames in [2, 10, 50]:
            ...
            hdu1 = fits.ImageHDU(obj[0, :, :])    
            hdul = fits.HDUList([hdu0, hdu1])
            hdul.writeto(f'gband_marginal_nimages_{n_frames}.fits', overwrite=True)
