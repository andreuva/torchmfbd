import numpy as np
import torch
import matplotlib.pyplot as pl
from tqdm import tqdm
import torchmfbd
from astropy.io import fits


if __name__ == '__main__':

    to_do = ['joint']

    s_u = 1000.0
    n_frames = 20

    if 'joint' in to_do:
        for n_modes in [44, 90, 230]:

            f = fits.open('../../reproducibility/obs/gband_bluec/gband_aligned.fits')

            cte = np.array([np.std(f[0].data[0, i, 0:200, 0:200]) / np.mean(f[0].data[0, i, 0:200, 0:200]) for i in range(f[0].data.shape[1])])

            ind = np.argsort(cte)[::-1]
            
            frames = f[0].data[:, ind[0:n_frames], :, :]
                
            # Convert to tensor
            frames = torch.tensor(frames.astype('float32'))
            
            frames = frames[:, :, 0:512, 0:512]

            frames /= torch.mean(frames[:, :, :, :], dim=(-2, -1), keepdim=True)

            f = open('gband_joint.yaml', 'r')
            lines = f.readlines()
            f.close()

            lines[13] = f"    s_u_joint: {s_u}\n"
            lines[33] = f"    nmax_modes : {n_modes}\n"
            
            f = open('tmp_joint.yaml', 'w')
            f.writelines(lines)
            f.close()
            
            # Create the deconvolution object                
            decSI = torchmfbd.Deconvolution('tmp_joint.yaml')

            # Patchify and add the frames
            patchify = torchmfbd.Patchify4D()
            frames_patches = patchify.patchify(frames, patch_size=96, stride_size=50, flatten_sequences=True)    
            noise = torchmfbd.compute_noise(frames_patches[0:1, 0:1, ...])
            decSI.add_frames(frames_patches, id_object=0, id_diversity=0, diversity=0.0, sigma=noise)

            # Deconvolve
            decSI.deconvolve(infer_object=False, 
                            optimizer='adam', 
                            simultaneous_sequences=1000,
                            n_iterations=450)
                    
            # Unpatchify
            obj = patchify.unpatchify(decSI.obj[0], apodization=6, weight_type='cosine', weight_params=30).cpu().numpy()

            f = fits.open('../../reproducibility/obs/gband_bluec/hifiplus1_20230721_085151_sd_speckle.fts')
            speckle = f[0].data

            npix = obj.shape[-1]
            
            # # Do plots
            # fig, ax = pl.subplots(nrows=2, ncols=3, figsize=(25, 15))
            # for i in range(2):
            #     ax[0, 0].imshow(frames[0, 0, 0:npix, 0:npix])
            #     ax[0, 1].imshow(obj[0, :, :])
            #     ax[0, 2].imshow(speckle[0:npix, 0:npix])

            #     ax[1, 0].imshow(frames[0, 0, 400:600, 300:500])
            #     ax[1, 1].imshow(obj[0, 400:600, 300:500])
            #     ax[1, 2].imshow(speckle[0:npix, 0:npix][400:600, 300:500])

            
            # Save the object as a fits file
            hdu0 = fits.PrimaryHDU(frames[0, 0, :, :])    
            hdu1 = fits.ImageHDU(obj[0, :, :])    
            hdul = fits.HDUList([hdu0, hdu1])
            hdul.writeto(f'gband_joint_nmodes_{n_modes}_su_{s_u}.fits', overwrite=True)

    if 'marginal' in to_do:
        for n_modes in [44, 90, 230]:

            f = fits.open('../../reproducibility/obs/gband_bluec/gband_aligned.fits')

            cte = np.array([np.std(f[0].data[0, i, 0:200, 0:200]) / np.mean(f[0].data[0, i, 0:200, 0:200]) for i in range(f[0].data.shape[1])])

            ind = np.argsort(cte)[::-1]
            
            frames = f[0].data[:, ind[0:n_frames], :, :]
                
            # Convert to tensor
            frames = torch.tensor(frames.astype('float32'))
            
            frames = frames[:, :, 0:512, 0:512]

            frames /= torch.mean(frames[:, :, :, :], dim=(-2, -1), keepdim=True)

            f = open('gband_marginal.yaml', 'r')
            lines = f.readlines()
            f.close()
            
            lines[33] = f"    nmax_modes : {n_modes}\n"
            
            f = open('tmp_marginal.yaml', 'w')
            f.writelines(lines)
            f.close()
            
            # Create the deconvolution object                
            decSI = torchmfbd.Deconvolution('tmp_marginal.yaml')

            # Patchify and add the frames
            patchify = torchmfbd.Patchify4D()
            frames_patches = patchify.patchify(frames, patch_size=96, stride_size=50, flatten_sequences=True)    
            noise = torchmfbd.compute_noise(frames_patches[0:1, 0:1, ...])
            decSI.add_frames(frames_patches, id_object=0, id_diversity=0, diversity=0.0, sigma=noise)

            # Deconvolve
            decSI.deconvolve(infer_object=False, 
                            optimizer='adam', 
                            simultaneous_sequences=1000,
                            n_iterations=450)
                    
            # Unpatchify
            obj = patchify.unpatchify(decSI.obj[0], apodization=6, weight_type='cosine', weight_params=30).cpu().numpy()

            f = fits.open('../../reproducibility/obs/gband_bluec/hifiplus1_20230721_085151_sd_speckle.fts')
            speckle = f[0].data

            npix = obj.shape[-1]
            
            # # Do plots
            # fig, ax = pl.subplots(nrows=2, ncols=3, figsize=(25, 15))
            # for i in range(2):
            #     ax[0, 0].imshow(frames[0, 0, 0:npix, 0:npix])
            #     ax[0, 1].imshow(obj[0, :, :])
            #     ax[0, 2].imshow(speckle[0:npix, 0:npix])

            #     ax[1, 0].imshow(frames[0, 0, 400:600, 300:500])
            #     ax[1, 1].imshow(obj[0, 400:600, 300:500])
            #     ax[1, 2].imshow(speckle[0:npix, 0:npix][400:600, 300:500])

            
            # Save the object as a fits file
            hdu0 = fits.PrimaryHDU(frames[0, 0, :, :])    
            hdu1 = fits.ImageHDU(obj[0, :, :])    
            hdul = fits.HDUList([hdu0, hdu1])
            hdul.writeto(f'gband_marginal_nmodes_{n_modes}.fits', overwrite=True)

            