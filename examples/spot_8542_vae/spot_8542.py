...
if __name__ == '__main__':
    ...
    decSI = torchmfbd.Deconvolution('vae.yaml')
    
    # Patchify and add the frames
    for i in range(2):        
        frames_patches = patchify.patchify(frames[:, i, :, :, :], patch_size=56, stride_size=32, flatten_sequences=True)
        decSI.add_frames(frames_patches, id_object=i, id_diversity=0, diversity=0.0)
                
    decSI.deconvolve(infer_object=False, 
                     optimizer='adam', 
                     simultaneous_sequences=120,
                     n_iterations=250)


    decSI2 = torchmfbd.Deconvolution('kl.yaml')
        
    # Patchify and add the frames
    for i in range(2):        
        frames_patches = patchify.patchify(frames[:, i, :, :, :], patch_size=56, stride_size=32, flatten_sequences=True)
        decSI2.add_frames(frames_patches, id_object=i, id_diversity=0, diversity=0.0)
                
    decSI2.deconvolve(infer_object=False, 
                        optimizer='adam', 
                        simultaneous_sequences=120,
                        n_iterations=250)
    ...
    for i in range(2):
        ...
        obj_kl.append(patchify.unpatchify(decSI2.obj[i], apodization=6, weight_type='cosine', weight_params=30).cpu().numpy())

    nx, ny = obj[0].shape[1], obj[0].shape[2]
    
    fig, ax = pl.subplots(nrows=2, ncols=3, figsize=(15, 10), sharex=True, sharey=True)
    for i in range(2):
        ax[i, 0].imshow(frames[0, i, 0, 0:, 0:], cmap='gray', interpolation='nearest')
        ax[i, 1].imshow(obj[i][0, 0, 0:, 0:], cmap='gray', interpolation='nearest')
        ax[i, 2].imshow(obj_kl[i][0, 0, 0:, 0:], cmap='gray', interpolation='nearest')
